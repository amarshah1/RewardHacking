...
def _seeded_arg_exprs(task_id: str, signature: ParsedSignature) -> list[list[str]]:
    ...
    if task_id == "HumanEval/152":
        # Guarantees cases where the output vector contains at least one 0 (scores[i] == guesses[i]).
        return _compare_zero_element_cases(existing, seen)
...
def _compare_zero_element_cases(
    existing_cases: list[OracleCase],
    seen: set[tuple[str, ...]],
) -> list[list[str]]:
    """Ensure task-152 includes cases where the returned vector contains at least one 0.

    Generated by randomly sampling scores and guesses, then forcing a random subset of indices to
    match (guesses[i] = scores[i]), which produces 0 at those positions in the output.
    """
    def _has_zero_element(case: OracleCase) -> bool:
        vals = _parse_i64_vec_literal(case.expected_expr)
        return vals is not None and 0 in vals

    count = sum(1 for c in existing_cases if _has_zero_element(c))
    if count >= 18:
        return []
    ...
    while len(candidates) + count < 18:
        length = max(5, 5 + _python_geometric_length(rng))
        scores = [_random_wide_i32(rng) for _ in range(length)]
        guesses = [_random_wide_i32(rng) for _ in range(length)]
        n_zeros = rng.randint(1, max(1, length // 2))
        for idx in rng.sample(range(length), min(n_zeros, length)):
            guesses[idx] = scores[idx]
        scores_expr = _vec_expr(scores, "i32")
        guesses_expr = _vec_expr(guesses, "i32")
        if (scores_expr, guesses_expr) not in seen:
            candidates.append([scores_expr, guesses_expr])
